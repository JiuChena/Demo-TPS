...
public class ResourcesLoadManager 
{
    ...
    /// <returns></returns>
    public T Load<T>(string path, string resName) where T : Object
    {
        T resource = Resources.Load<T>(path + resName);
    ...
    /// <typeparam name="T">资源类型</typeparam>
    public T LoadSync<T>(string path, string resName, UnityAction<T> callback = null) where T : Object
    {
        IEnumerator ie = LoadSyncCoroutine<T>(path + resName, callback);
        T res= null;
        while (ie.MoveNext())
        {
            if (ie.Current is T)
